    ...
    try
    {
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            else if (auto data = object.cast<vsg::Data>())
            {
                ...
                {
                    modelGroup->addChild(textureGeometry);
                }
            }
            else if (object)
            {
        ...
        modelGroup->accept(computeBounds);
        if (!computeBounds.bounds.valid())
        {
            computeBounds.bounds.min = vsg::vec3(-1.0, -1.0, -1.0);
            computeBounds.bounds.max = vsg::vec3(1.0, 1.0, 1.0);
        }
        ...
        auto renderGraph = commandGraph->children[0].cast<vsg::RenderGraph>();
        if (renderGraph)
        {
            renderGraph->setClearValues({{0.0f, 0.0f, 0.0f}});
        }
